...
sns.set_theme(palette="deep", font_scale=1.7)


def plot_pixel_intensities(
    model_type: Literal["vae", "wgan"],
    label: Literal["experiment_4", "experiment_5"],
    gen_nrs: list[int] | int,
    n_images: int = 10_000,
) -> None:
    ...
    for gen_nr in gen_nrs:
        images = generate_data(model_type, gen_nr, n_images, label)
    ...
    for intensities, gen_nr in zip(intensities_list, gen_nrs):
        counts, bin_edges = np.histogram(intensities, bins=200, range=(-0.2, 1), density=True)
    ...
    plt.xlim(-0.2, 1.1)
    plt.savefig(
        f"plots/pixel_intensity_{model_type}_{label}.pdf", bbox_inches="tight", pad_inches=0
    )
    plt.show()
...
def main():
    n_images = 20_000
    gens = [0, 10, 19]
    plot_pixel_intensities("vae", "experiment_4", gens, n_images)
    plot_pixel_intensities("wgan", "experiment_4", gens, n_images)
    plot_pixel_intensities("wgan", "experiment_5", gens, n_images)
